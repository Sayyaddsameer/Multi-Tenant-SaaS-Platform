...
The project is structured as a \*\*Monorepo\*\* containing both the Backend API and the Frontend React application, orchestrated by Docker Compose at the root level.
...
├── docker-compose.yml       # Defines services (db, backend, frontend) \& networking
...
│   │   ├── authMiddleware.js      # JWT verification \& Tenant ID extraction
...
│   └── entrypoint.sh        # Startup script for Docker (runs migrations/seeds)
...
│   ├── pages/               # Full page views matching routes
...
├── nginx.conf               # Nginx config for serving static files in Docker
...
\- Use the credentials provided in `submission.json` to test the authentication endpoints.
...
docker exec -it database psql -U postgres -d saas\_db
